...
Domain = {
        "assign": Operator(head=('assign', V('x'), V('y')),
                           conditions=Fact(field=V('x'), value=V('vx')),
        ...
        "fracMult": Method(head=('fracMult', V('xn'), V('yn'), V('xd'), V('yd')),
                            conditions=[(Fact(field=V('xn'), value=V('vnx'))&Fact(field=V('yn'), value=V('vny'))&Fact(field=V('xd'), value=V('vxd'))&Fact(field=V('yd'), value=V('vyd')))],
                            subtasks=[[Task(head=('intMult', 'xn', 'yn', 'nom'), primitive=True), Task(head=('intMult', 'xd', 'yd', 'denom'), primitive=True)]]),
                            
        "add": Method(head=('add',),
                      conditions=[(Fact(field=V('xn'), value=V('vnx'))&Fact(field=V('yn'), value=V('vny'))&Fact(field=V('xd'), value=V('vdx'))&Fact(field=V('yd'), value=V('vdy'))&Filter(lambda xn,yn,xd,yd: xn=='xn' and yn=='yn' and xd=='xd' and yd=='yd')),
        ...
        "solve":  Method(head=('solve',),
                     conditions=[Fact(field=V('op'), operator='+'), Fact(field=V('op'), operator='*')],
...
if __name__ == "__main__":
    ...
    operator = "+"
    state = Fact(field='op', operator=operator)&Fact(field='xn', value=numerator_x)&Fact(field='yn', value=numerator_y)&Fact(field='xd', value=denominator_x)&Fact(field='yd', value=denominator_y)
